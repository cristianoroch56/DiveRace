...
class DUP_PRO_Migration
{
    ...
    public static function renameInstallersPhpFiles()
    {
        $pathsTocheck = array(
            ...
            DupProSnapLibIOU::safePathTrailingslashit(DupProSnapLibUtilWp::getHomePath()),
            DupProSnapLibIOU::safePathTrailingslashit(WP_CONTENT_DIR)
        );

        $migrationData = self::getMigrationData();
        if (isset($migrationData['installerPath'])) {
            $pathsTocheck[] = DupProSnapLibIOU::safePathTrailingslashit(dirname($migrationData['installerPath']));
        }
        if (isset($migrationData['dupInstallerPath'])) {
            $pathsTocheck[] = DupProSnapLibIOU::safePathTrailingslashit(dirname($migrationData['dupInstallerPath']));
        }
        ...
        foreach ($pathsTocheck as $cFolder) {
            ...
            if (
                !is_file($cFile) ||
                !DupProSnapLibIOU::chmod($cFile, 'u+rw') ||
                !is_readable($cFile)
            ) {
                continue;
            }
        ...
        foreach ($filesToCheck as $file) {
            ...
            if (strpos($file, $archiveHash) === false) {
                if (DupProSnapLibIOU::rename($file, dirname($file) . '/' . $archiveHash . '_installer.php', true)) {
                    self::$migrationCleanupReport['instFile'][] = "<div class='failure'>"
                        . "<i class='fa fa-check green'></i> "
